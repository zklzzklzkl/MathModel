...
### Phase 6: 修订闭环 → [[skills/mm-revision-integrator/SKILL.md]]
...
### Phase 7: 最终验收 → [[skills/mm-final-verify/SKILL.md]]

```
┌─ 运行自动审计:
│   └── python scripts/audit_v2_run.py --workspace <path>
...
| 参考文件                                 | Phase 0 | Phase 1 | Phase 2 | Phase 3 | Phase 4 | Phase 5 | Phase 6 | Phase 7 |
| ------------------------------------ | :-----: | :-----: | :-----: | :-----: | :-----: | :-----: | :-----: | :-----: |
...
赛题文件 (PDF/DOCX/CSV)
  │
  ├─→ [Phase 1: mm-problem-intake]
  │     ├─→ PROBLEM_BRIEF.md   ← 题面结构化
  │     └─→ DATA_AUDIT.md      ← 数据质量审计
  │
  ├─→ [Phase 2: mm-model-strategy]
  ...
  │
  ├─→ [Phase 5: mm-contest-review]
  │     ├─→ PAPER_SCORECARD.md    ← 10 维度评分
  │     └─→ REVISION_ACTIONS.md   ← 修订清单
  │
  ├─→ [Phase 6: mm-revision-integrator]
  │     └─→ REVISION_STATUS.md    ← 修订状态追踪
  │
  └─→ [Phase 7: mm-final-verify]
